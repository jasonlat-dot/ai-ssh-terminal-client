...
import { Connections, loadHosts } from './components/Connections';
...
export default function App() {
  ...
  const selectHost = (id: string) => {
    ...
    if (!nextHost.online) notify(`${nextHost.name} 尚未连接；真实 SSH 服务待接入。`);
  };
  ...
  const saveHosts = (next: Host[]) => {
    try { localStorage.setItem('agent-ssh-hosts-v1', JSON.stringify(next)); }
    catch { notify('无法保存连接配置，请检查本地存储空间或浏览器权限。'); return false; }
    setHosts(next);
    setSessions(previous => previous.map(session => {
      const configured = next.find(item => item.id === session.hostId);
      return configured ? { ...session, title: configured.name } : session;
    }));
    return true;
  };
  ...
  return <div style={{ '--agent-width': `${agentWidth}%` } as CSSProperties} className={`app-shell ${maximized ? 'terminal-maximized' : ''} ${navCollapsed ? 'nav-collapsed' : ''} ${navigation === '连接' ? 'connections-view' : 'terminal-view'}`}>
    ...
    {navigation === '连接' && <Connections hosts={hosts} save={saveHosts} terminal={selectHost} local={newSession} copy={copy} />}
