...
import { createSessionFileState, initialCommands, mockReply, mockTransferProgress } from './data/mock';
...
type Dialog = 'command' | 'connection' | 'file' | 'search' | 'settings' | null;
...
export default function App() {
  ...
  return <div style={{ '--agent-width': `${agentWidth}%` } as CSSProperties} className={`app-shell ${maximized ? 'terminal-maximized' : ''} ${navCollapsed ? 'nav-collapsed' : ''} ${navigation === '连接' ? 'connections-view' : 'terminal-view'}`}>
    ...
    <AgentPanel host={host} messages={messages} busy={chatBusy} send={sendMessage} clear={clearChat} confirm={confirm} setConfirm={setConfirm} runLogs={() => requestRun('tail -f logs/error.log')} notify={notify} disabled={!canRun} connect={openNewConnection} manageConnections={() => navigate('连接')} openFiles={openFiles} openCommands={() => { navigate('命令'); setCommandCollapsed(false); setFocusTick(value => value + 1); }} settings={() => navigate('设置')} />
    <StatusBar host={host} onInfo={() => notify('SSH 远程终端已对接后端；SFTP 与智能体仍为模拟功能。')} />
