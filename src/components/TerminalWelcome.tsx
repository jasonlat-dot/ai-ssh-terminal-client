  ...
  return <>
    <section className="terminal-welcome" aria-labelledby="terminal-welcome-title">
      <div className="terminal-welcome-content">
        ...
        <p className="welcome-demo-note">顶部“文件上传”可独立上传和下载文件；SSH 服务器目录与 SFTP 传输尚未接入。</p>
      </div>
    </section>
    {guideOpen && <Modal title="终端连接指引" onClose={() => setGuideOpen(false)}>
      <div className="connection-guide-content">
        ...
        <ol>
          ...
          <li><strong>上传与下载文件</strong><span>点击顶部“文件上传”选择本地文件，上传完成后可通过临时链接下载。上传文件不会自动发送给 Agent，也不会写入 SSH 服务器目录。</span></li>
        </ol>
        <p className="muted">SSH 连接配置保存在后端，收藏偏好保存在本地。文件面板中的目录目前为演示数据。</p>
