...
export function AppHeader({ search, notify }: {
  ...
  return <header className="app-header">
    ...
    <button className="project-select" onClick={() => notify('当前演示项目：Web 服务')}><Icon name="terminal" size={15} /><span>Web 服务</span><Icon name="down" size={14} /></button>
...
export function ActivityBar({ active, onSelect, onSettings, settingsOpen, collapsed, toggleCollapsed }: {
  ...
  return <nav className="activity-bar" aria-label="主导航">
    ...
    <button className="activity" aria-label="客户端设置" title={collapsed ? '客户端设置' : undefined} aria-haspopup="dialog" aria-expanded={settingsOpen} onClick={onSettings}><Icon name="settings" size={23} /><span><strong>设置</strong><small>后端服务器</small></span></button>
